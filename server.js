...
try {
await pool.query(  CREATE TABLE IF NOT EXISTS deposit_requests (   id SERIAL PRIMARY KEY,   request_id TEXT UNIQUE,   email TEXT NOT NULL,   plan_name TEXT,   network_code TEXT NOT NULL,   network_name TEXT NOT NULL,   deposit_address TEXT NOT NULL,   amount NUMERIC NOT NULL DEFAULT 0,   txid TEXT,   receipt_image TEXT,   status TEXT DEFAULT 'قيد الدفع',   review_note TEXT DEFAULT '',   created_at TIMESTAMP DEFAULT NOW(),   expires_at TIMESTAMP,   submitted_at TIMESTAMP,   reviewed_at TIMESTAMP   )  );
...
try {
await pool.query(  CREATE TABLE IF NOT EXISTS users (   id SERIAL PRIMARY KEY,   name TEXT,   email TEXT UNIQUE,   password TEXT,   verified BOOLEAN DEFAULT false   )  );
...
try {
await pool.query(  UPDATE deposit_requests   SET status = 'ملغي', review_note = 'انتهت مدة الطلب', reviewed_at = NOW()   WHERE status = 'قيد الدفع' AND expires_at IS NOT NULL AND expires_at < NOW()  );
} catch (error) {
...
try {
const result = await pool.query(
  SELECT   name,   email,   verified,   has_deposited,   withdraw_password,   vip_level,   verification_status,   verification_type,   balance   FROM users   WHERE email = $1  ,
[cleanEmail]
);
...
try {
const result = await pool.query(
  SELECT   verification_status,   verification_type,   verification_full_name,   verification_document_number,   verification_birth_date,   verification_expiry_date,   verification_country,   verification_front_image,   verification_back_image,   verification_passport_image,   verification_submitted_at   FROM users   WHERE email = $1  ,
[cleanEmail]
);
